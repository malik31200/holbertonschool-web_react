  ...
  test('checks that submit button is enabled only with valid email and password', async () => {
  const user = userEvent.setup();
  render(<Login />);
...
test('checks that logIn is called with email and password on submit', async () => {
  const user = userEvent.setup();
  const logIn = jest.fn();
